...
export default async function AdminLessonsPage(props: { searchParams: Promise<{ courseId?: string }> }) {
  const searchParams = await props.searchParams;
  const allCourses = await db.query.courses.findMany();

  let activeCourseId = searchParams.courseId || null;

  if (!activeCourseId && allCourses.length > 0) {
